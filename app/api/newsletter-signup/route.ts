...
export async function POST(request: Request) {
	try {
		...
		const response = await fetch(`https://${process.env.SHOPIFY_STORE_DOMAIN}/admin/api/2024-01/customers/marketing_consent.json`, {
			method: "POST",
			headers: {
		...
		try {
			...
			// Ignore customer creation errors as they might already exist
