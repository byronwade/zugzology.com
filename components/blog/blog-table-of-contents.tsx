...
export function BlogTableOfContents({ contentHtml }: TableOfContentsProps) {
	...
	const handleHeadingClick = (e: React.MouseEvent<HTMLAnchorElement>, id: string) => {
		...
		if (!targetElement) {
			console.log(`Element with ID "${id}" not found. Searching by content...`);
	...
	return (
		<div className="blog-table-of-contents" data-orientation="vertical" data-state={isExpanded ? "open" : "closed"}>
			<h3 data-orientation="vertical" data-state={isExpanded ? "open" : "closed"} className="flex">
				<button type="button" onClick={() => setIsExpanded(!isExpanded)} className="flex flex-1 items-center justify-between text-sm font-medium transition-all text-left [&[data-state=open]>svg]:rotate-180 hover:no-underline">
					...
					<div className="px-4 py-3">
						<ul className="space-y-2.5">
							{headings.map((heading) => (
								<li key={heading.id}>
									<a href={`#${heading.id}`} className={cn("block text-sm transition-colors duration-200", heading.level === 2 ? "pl-0" : "pl-4", activeId === heading.id ? "text-purple-600 dark:text-purple-400 font-medium" : "text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-300")} onClick={(e) => handleHeadingClick(e, heading.id)}>
										{heading.text}
									</a>
								</li>
							))}
