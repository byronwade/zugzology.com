...
export const ProductCard = memo(function ProductCard({
	...
	const handleAddToCart = useCallback(
		async (e: React.MouseEvent) => {
			...
			try {
				...
				// Track cart add for AI
				trackInteraction({
					productId: product.id,
					type: 'cart_add',
					context: `product-card-${view}`
				});
	...
	}, [isWishlisted, product.handle, product.id, removeFromWishlist, addToWishlist]);
	...
	const productImages = useMemo(() => {
		...
		// Also check if there are featured images or other image fields
		...
		if (product.variants?.nodes) {
			product.variants.nodes.forEach(variant => {
				if (variant.image?.url && !images.includes(variant.image.url)) {
					...
					console.log(`  Variant Image: ${variant.image.url}`);
				}
			});
		}
		
		// Image logging removed
		return images;
	}, [product]);
	...
	return (
		<div
			...
			<Link 
				href={productUrl} 
				className={cn("block shrink-0", view === "grid" ? "w-full" : "w-24 sm:w-32")}
				onClick={() => {
					// Track product view for AI
					trackInteraction({
						productId: product.id,
						type: 'view',
						context: `product-card-${view}-image`
					});
				}}
			>
				<div
				...
				>
					{productDetails.imageUrl ? (
						<Image
							...
							onLoad={(event) => {
								// Disabled prefetching to prevent excessive resource warnings
				...
				<Link 
					...
					onClick={() => {
						...
						trackInteraction({
							productId: product.id,
							type: 'view',
							context: `product-card-${view}-title`
						});
					}}
				>
					{/* Vendor */}
