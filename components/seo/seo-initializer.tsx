...
export function SEOInitializer() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(monitorSEOHealth, 300000); // Every 5 minutes
  ...
  try {
    ...
    scripts.forEach((script, index) => {
      try {
        ...
        console.log(`[SEO] Structured Data ${index + 1}:`, data);
        
        // Basic validation
        if (!data['@context']) {
          console.warn(`[SEO] Missing @context in structured data ${index + 1}`);
        }
        if (!data['@type'] && !data['@graph']) {
          console.warn(`[SEO] Missing @type or @graph in structured data ${index + 1}`);
        }
      } catch (error) {
        console.error(`[SEO] Invalid JSON-LD in script ${index + 1}:`, error);
      }
    });
    
    if (scripts.length === 0) {
      console.warn('[SEO] No structured data found on page');
    } else {
      console.log(`[SEO] Found ${scripts.length} structured data scripts`);
    }
  } catch (error) {
    console.error('[SEO] Structured data validation error:', error);
  }
}
...
async function monitorSEOHealth() {
  try {
    ...
    if (data.score < 60) {
      console.warn('[SEO] Health score is low:', data.score);
      console.warn('[SEO] Recommendations:', data.recommendations);
    }
  } catch (error) {
