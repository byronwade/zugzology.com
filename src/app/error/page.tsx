...
export default async function AuthErrorPage({
	...
	const errorMessages: Record<string, { title: string; description: string; solution?: string }> = {
		Configuration: {
			...
			solution:
				"This is likely due to missing or incorrect environment variables. Please check that all required Shopify credentials are properly set.",
		},
		AccessDenied: {
			title: "Access Denied",
		...
		invalid_client: {
			...
			solution:
				"This is likely due to incorrect Shopify API credentials or a mismatch between the redirect URI configured in Shopify and the one used in the request.",
		},
		Default: {
	...
	const detectedErrorType = lowerCaseError.includes("invalid_client")
		? "invalid_client"
		: lowerCaseError.includes("configuration")
			? "Configuration"
			: errorType;
			...
			<Card className="w-full max-w-md">
				<CardHeader className="space-y-1">
					<div className="mb-2 flex items-center space-x-2 text-amber-500">
						...
						<span className="font-medium text-sm">Authentication Issue</span>
					</div>
					<CardTitle className="font-bold text-2xl">{errorInfo.title}</CardTitle>
					<CardDescription>{errorInfo.description}</CardDescription>
					{errorInfo.solution && <p className="mt-2 text-muted-foreground text-sm">{errorInfo.solution}</p>}
				...
				<CardContent className="grid gap-6">
					<div className="grid gap-4">
						...
						<div className="text-center text-muted-foreground text-sm">
							...
							<Link className="underline underline-offset-4 hover:text-primary" href="/help">
								customer support
							</Link>
							.
						</div>
