...
import { getPaginatedProducts } from "@/lib/actions/shopify";
...
function optimizeProduct(product: any) {
	...
	const compareAtPrice = firstVariant?.compareAtPrice ? firstVariant.compareAtPrice.amount : null;

	// Check if product is on sale
	const isOnSale = compareAtPrice ? Number.parseFloat(price) < Number.parseFloat(compareAtPrice) : false;
