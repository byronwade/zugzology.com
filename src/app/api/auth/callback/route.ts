	...
	try {
		...
		if (error) {
			...
			return Response.redirect(
				`${process.env.NEXT_PUBLIC_APP_URL}/login?error=${encodeURIComponent(errorDescription || "Authentication failed")}`
			);
		}
		...
		formData.append("client_id", process.env.SHOPIFY_CLIENT_ID || "");
		...
		if (!tokenResponse.ok) {
			...
			return Response.redirect(
				`${process.env.NEXT_PUBLIC_APP_URL}/login?error=Authentication failed: ${encodeURIComponent(errorText)}`
			);
		}
		...
		return Response.redirect(
			`${process.env.NEXT_PUBLIC_APP_URL}/login?error=${encodeURIComponent(error instanceof Error ? error.message : "Authentication failed")}`
		);
	}
}
