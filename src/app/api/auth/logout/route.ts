...
import { createErrorResponse, verifyAuthentication } from "@/lib/server/api-utils";
...
export async function POST() {
	try {
		...
		logAuthEvent("Logout completed", {
			clearedCookies: [
				AUTH_CONFIG.cookies.customerAccessToken.name,
				AUTH_CONFIG.cookies.accessToken.name,
				AUTH_CONFIG.cookies.idToken.name,
				AUTH_CONFIG.cookies.csrfToken.name,
				...cartCookies.map((c) => c.name),
			],
		});
