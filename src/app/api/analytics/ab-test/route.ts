...
const MAX_EVENTS = 10_000; // Limit memory usage
...
export async function POST(request: NextRequest) {
	try {
		...
		return NextResponse.json({ error: "Unable to track AB test event" }, { status: 500 });
	}
}
...
export async function GET(request: NextRequest) {
	try {
		...
		return NextResponse.json({ error: "Unable to get AB test data" }, { status: 500 });
	}
}

function getTestSummary(testId?: string | null) {
	const events = testId ? abTestEvents.filter((e) => e.testId === testId) : abTestEvents;
	...
	Object.keys(summary).forEach((testId) => {
		Object.keys(summary[testId]).forEach((variantId) => {
			...
			variant.events = undefined; // Remove detailed events from summary
		});
	});
...
function getTestEvents(testId?: string | null) {
	...
	const sortedEvents = events.sort((a, b) => b.timestamp - a.timestamp).slice(0, 1000); // Limit to last 1000 events
...
export async function DELETE(request: NextRequest) {
	try {
		...
		return NextResponse.json({ error: "Unable to delete AB test data" }, { status: 500 });
	}
}
