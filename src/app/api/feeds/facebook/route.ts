...
function generateFacebookCatalogCSV(products: ShopifyProduct[], baseUrl: string, config: any): string {
...
function generateFacebookRow(product: ShopifyProduct, variant: any, baseUrl: string, config: any): string {
	...
	const fields = [
		...
		"unisex", // gender
		"all ages", // age_group
		variant.selectedOptions?.find((o: { name: string; value: string }) => o.name === "Color")?.value || "", // color
		variant.selectedOptions?.find((o: { name: string; value: string }) => o.name === "Size")?.value || "", // size
		"US::Standard::0 USD", // shipping
		product.tags?.[0] || "", // custom_label_0
		product.productType || "", // custom_label_1
		product.collections?.edges?.[0]?.node?.title || "", // custom_label_2
