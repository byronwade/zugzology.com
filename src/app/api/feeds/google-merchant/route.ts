...
export async function GET() {
	try {
		...
		const _collectionMap = new Map(collections.map((c) => [c.id, c.title]));
...
function generateGoogleMerchantFeed(products: ShopifyProduct[], baseUrl: string, config: any): string {
    ...
    <item>
      ...
      <g:shipping_weight>${variant?.weight || "1"} ${variant?.weightUnit || "lb"}</g:shipping_weight>
      <g:custom_label_0>${product.tags?.[0] || ""}</g:custom_label_0>
      <g:custom_label_1>${product.collections?.edges?.[0]?.node?.title || ""}</g:custom_label_1>
