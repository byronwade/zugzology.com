...
const getMetafieldValue = (
	...
	namespace: string,
	key: string
): string | undefined => metafields?.find((field) => field.namespace === namespace && field.key === key)?.value;

// Helper function to get rating data
const getRatingData = (product: TransformedProduct) => {
	const rating = Number.parseFloat(getMetafieldValue(product.metafields, "custom", "rating") || "0");
	const ratingCount = Number.parseInt(getMetafieldValue(product.metafields, "custom", "rating_count") || "0", 10);
	const recentPurchases = Number.parseInt(
		getMetafieldValue(product.metafields, "custom", "recent_purchases") || "0",
		10
	);
...
const _calculateTotalQuantity = (variants: { edges: Array<{ node: ShopifyProductVariant }> }): number => {
	let total = 0;
...
export function ProductRecommendations({
	...
	const [_historyProducts, setHistoryProducts] = useState<TransformedProduct[]>([]);
	...
	const popularProducts = featuredProducts
		.filter((product: TransformedProduct) => {
			const recentPurchases = product.metafields?.find(
				(field: ShopifyMetafield) => field.key === "recent_purchases"
			)?.value;
			return recentPurchases && Number.parseInt(recentPurchases, 10) > 10;
		})
		.slice(0, 12)
		.map((product) => ({
			product: transformProduct(product),
				...
				<div className="animate-pulse space-y-4">
					...
					<div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
						{Array.from({ length: 5 }).map((_, i) => (
							<div className="aspect-square rounded bg-neutral-200 dark:bg-neutral-800" key={i} />
						))}
					</div>
				</div>
			</div>
		...
		<div className="space-y-12">
			...
			{limitedFeaturedProducts.length > 0 && (
				<HistoryRecommendations
					...
					products={limitedFeaturedProducts.map(transformProduct)}
				/>
			)}
			...
			{relatedProducts.length > 0 && (
				<ProductSection
					currentProductId={currentPost.id}
					description="Products you might be interested in based on this article"
			...
			{bestSellers.length > 0 && (
				<ProductSection
					currentProductId={currentPost.id}
					description="Our most popular products, loved by cultivators"
			...
			{newArrivals.length > 0 && (
				<ProductSection
					currentProductId={currentPost.id}
					description="Our latest products and additions"
			...
			{complementaryProducts.length > 0 && (
				<ProductSection
					currentProductId={currentPost.id}
					description="Products that complement your interests"
			...
			{seasonalPicks.length > 0 && (
				<ProductSection
					currentProductId={currentPost.id}
					description="Products perfect for this time of year"
