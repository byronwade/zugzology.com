...
export async function getSearchData(query: string, sort = "featured", page = 1) {
	...
	const filteredProducts = products.filter((product) => {
		...
		const searchableText = [product.title, product.productType, product.vendor, ...(product.tags || [])]
			.filter(Boolean)
			.join(" ")
			.toLowerCase();
		...
		const result =
			basicMatch ||
			(product.description && searchTerms.every((term) => product.description?.toLowerCase().includes(term)));
