...
export function useExpensiveMemo<T>(factory: () => T, deps: React.DependencyList, debugName?: string): T {
	return useMemo(() => {
...
export function useIntersectionObserver(targetRef: React.RefObject<Element>, options: IntersectionObserverInit = {}) {
...
export function useOptimisticState<T>(initialState: T, updateFn: (newState: T) => Promise<T>) {
...
export function logBundleSize(_componentName: string) {
