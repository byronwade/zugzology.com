...
import { BRAND, CONTACT, INDUSTRY, SOCIAL } from "@/lib/config/wadesdesign.config";
import type { ShopifyProduct } from "@/lib/types";

// Base Organization data
export const getOrganizationSchema = (siteSettings?: any): WithContext<Organization> => {
	const storeName = siteSettings?.name || BRAND.name;
	const storeUrl = siteSettings?.url || BRAND.url;
	const logo = siteSettings?.logo || `${BRAND.url}${BRAND.logo.path}`;
	const phone = process.env.NEXT_PUBLIC_CONTACT_PHONE || CONTACT.phone.mainFormatted;
	...
	return {
		...
		sameAs: [SOCIAL.facebook, SOCIAL.twitter, SOCIAL.instagram, SOCIAL.pinterest].filter(Boolean),
		description: `Premium ${INDUSTRY.terminology.mainProduct} and equipment provider. Quality growing kits, substrates, and expert guidance.`,
		contactPoint: [
			{
				"@type": "ContactPoint",
				telephone: phone,
...
export const getStoreSchema = (siteSettings?: any): WithContext<Store & LocalBusiness> => {
	const storeName = siteSettings?.name || BRAND.name;
	const storeUrl = siteSettings?.url || BRAND.url;
	const logo = siteSettings?.logo || `${BRAND.url}/logo.png`;
...
export const getProductSchema = (product: ShopifyProduct): WithContext<Product> => {
	...
	const schema: WithContext<Product> = {
		...
		offers: {
			...
			offers: variants.map((variant) => ({
				...
				url: `${BRAND.url}/products/${product.handle}?variant=${variant.id}`,
	...
	if (Number.parseInt(reviewCount, 10) > 0 && Number.parseFloat(ratingValue) > 0) {
		schema.aggregateRating = {
			"@type": "AggregateRating",
			ratingValue: Number.parseFloat(ratingValue).toFixed(1),
			reviewCount: Number.parseInt(reviewCount, 10),
			bestRating: "5",
			worstRating: "1",
		};
	}
...
export const getCollectionSchema = (
	...
	const collectionUrl = `${BRAND.url}/collections/${collection.handle}`;
...
export const getBlogSchema = (blog: any, articles: any[] = []): WithContext<Blog> => {
	const blogUrl = `${BRAND.url}/blogs/${blog.handle}`;
	...
	return {
		...
		...(articles.length > 0 && {
			blogPost: articles.map((article) => ({
				...
				author: {
					"@type": "Person",
					name: article.author?.name || `${BRAND.name} Team`,
				},
...
export const getBlogPostSchema = (blog: any, article: any): WithContext<BlogPosting> => {
	const articleUrl = `${BRAND.url}/blogs/${blog.handle}/${article.handle}`;
