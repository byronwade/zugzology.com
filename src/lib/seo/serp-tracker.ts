...
export class SERPTracker {
	...
	private readonly trackingHistory: Map<string, RankingHistory> = new Map();
	...
	async trackKeyword(
		...
		try {
			...
			const ourResult = serpData.organicResults.find((result: any) => result.url.includes(this.domain));
	...
	generateRankingAlerts(): Array<{
		...
		this.trackingHistory.forEach((history, keyword) => {
			...
			const previous = history.positions.at(-2);
	...
	private updateTrackingHistory(keyword: string, position: number | null): void {
		...
		const previousPosition = history.positions.length > 0 ? history.positions.at(-1).position : null;

		const changeFromPrevious = previousPosition && position ? previousPosition - position : 0;
		...
		history.positions = history.positions.filter((pos) => pos.date >= thirtyDaysAgo);
	}
		...
		const mockResults = {
			...
			featuredSnippet: keyword.includes("how")
				? {
						...
						snippet:
							"To grow mushrooms, you need a sterile substrate, mushroom spawn, and proper environmental conditions...",
					}
				: undefined,
			images: ["https://example.com/mushroom1.jpg", "https://example.com/mushroom2.jpg"],
			videos: [
				{
	...
	generateKeywordOpportunities(serpFeatures: SERPFeatures): string[] {
		...
		if (serpFeatures.peopleAlsoAsk.length > 0) {
			opportunities.push(`Create FAQ content targeting: ${serpFeatures.peopleAlsoAsk.slice(0, 3).join(", ")}`);
		}

		// Opportunities from Related Searches
		if (serpFeatures.relatedSearches.length > 0) {
			opportunities.push(`Target related keywords: ${serpFeatures.relatedSearches.slice(0, 3).join(", ")}`);
		}

		// Featured Snippet opportunity
		if (serpFeatures.featuredSnippet && !serpFeatures.featuredSnippet.source.includes(this.domain)) {
			opportunities.push(`Optimize for featured snippet: Structure content as ${serpFeatures.featuredSnippet.type}`);
		}
...
export class SERPFeatureOptimizer {
	...
	private hasDirectAnswer(content: string, query: string): boolean {
		...
		const answerPatterns = [
			...
			/the\s+answer\s+is/i,
			new RegExp(`${queryWords.at(-1)}\\s+is\\s+`, "i"),
		];
