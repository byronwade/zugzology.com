...
import { ShopifyProduct, ShopifyCollection, ShopifyBlogArticle } from "@/lib/types";
...
export const getEnhancedStoreSchema = (): WithContext<Store & LocalBusiness> => {
  ...
  return {
    ...
    slogan: config.slogan || "Premium Mushroom Cultivation Supplies",
    makesOffer: {
      "@type": "Offer",
      itemOffered: {
        "@type": "Service",
...
export const getEnhancedProductSchema = (product: ShopifyProduct): WithContext<Product> => {
  ...
  const reviewCount = parseInt(product.metafields?.find(m => m?.key === "review_count")?.value || "0");
  const rating = parseFloat(product.metafields?.find(m => m?.key === "rating")?.value || "0");
    ...
    schema.review = [
      {
        ...
        datePublished: "2024-01-01T00:00:00Z", // Fixed date to avoid dynamic generation
      },
    ];
  }
 ...
 */
export const getEnhancedBlogPostSchema = (
 ...
 */
export const getSpecialAnnouncementSchema = (
  announcement: {
    name: string;
    text: string;
    datePosted: string;
...
export const getEnhancedOrderSchema = (order: {
  ...
  return {
    ...
    totalPrice: {
      "@type": "MonetaryAmount",
      value: parseFloat(order.totalAmount.amount),
