...
export class LinkHealthMonitor {
  private linkCache: Map<string, LinkStatus> = new Map();
   ...
   */
  private generateFixSuggestion(url: string, statusCode: number): string {
    switch (statusCode) {
      case 404:
...
export class ContentDecayMonitor {
  ...
  analyzeContentFreshness(
    ...
    if (daysSinceUpdate > 365) decayScore += 40;
    else if (daysSinceUpdate > 180) decayScore += 30;
    else if (daysSinceUpdate > 90) decayScore += 20;
    else if (daysSinceUpdate > 30) decayScore += 10;
    
    // Traffic decline factor (max 30 points)
    if (trafficTrend < -50) decayScore += 30;
    else if (trafficTrend < -25) decayScore += 20;
    else if (trafficTrend < 0) decayScore += 10;
    
    // Ranking decline factor (max 30 points)
    if (rankingTrend > 10) decayScore += 30;
    else if (rankingTrend > 5) decayScore += 20;
    else if (rankingTrend > 0) decayScore += 10;
    
    // Determine status
    let status: 'fresh' | 'declining' | 'stale';
    if (decayScore < 30) status = 'fresh';
    else if (decayScore < 60) status = 'declining';
    else status = 'stale';
