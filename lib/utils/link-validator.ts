...
import { getAllCollections, getProducts, getBlogs, getAllBlogPosts } from "@/lib/api/shopify/actions";
import { CACHE_TAGS, CACHE_TIMES } from "@/lib/api/shopify/cache-config";
import { ShopifyCollection, ShopifyProduct, ShopifyBlogArticle } from "@/lib/types";
...
export const getLinkMappings = unstable_cache(
	async (): Promise<LinkMappings> => {
		console.log("Building link mappings from Shopify data...");
		...
		specialBlogPosts.forEach((path) => {
			if (!blogPostMappings[path]) {
				...
				blogPostMappings[path] = {
					exists: false,
					actualLink:
						alternativePost && alternativePost.blog
