...
class ConnectionManager {
  ...
  private connectionChangeCallbacks: Array<(settings: AdaptiveSettings) => void> = [];
  ...
  private calculateOptimalSettings(): AdaptiveSettings {
    ...
    if (network.saveData) {
      settings = {
        ...
        enablePrefetch: false,
        quality: 'low',
        maxImageSize: Math.min(settings.maxImageSize, 100000)
      };
    }
    ...
    if (device.memory < 4) {
      settings = {
        ...
        maxImageSize: Math.min(settings.maxImageSize, 500000)
      };
    }
